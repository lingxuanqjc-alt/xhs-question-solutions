...
export const DisclosureScene = ({scene, video}) => {
  const {emphasisStyleFor} = useFocusMotion(4, sceneFocusInterval(scene, 4));
  return <SceneShell scene={scene} video={video} eyebrow="证据与披露">
    <Reveal><SceneTitle>可以参考，但不能跳过复核</SceneTitle></Reveal>
    <div className="disclosure-list">
      <div className="focus-emphasis" style={emphasisStyleFor(0)}><FieldCard label="样本与来源" delay={8}>
        {scene.content.source_label} · {scene.content.is_truncated ? "评论未完整采集" : "按采集范围整理"}<br />
        {scene.content.coverage}
      </FieldCard></div>
      <div className="focus-emphasis" style={emphasisStyleFor(1)}><FieldCard label="生成方式" delay={14}>AI 辅助整理 · 经验不等于事实</FieldCard></div>
      <div className="focus-emphasis" style={emphasisStyleFor(2)}><FieldCard label="利益关系" delay={20}>未知，发布前人工确认</FieldCard></div>
      <div className="focus-emphasis" style={emphasisStyleFor(3)}><FieldCard label="发布状态" tone={scene.content.publish_status === "ready" ? "safe" : "warning"} delay={26}>{scene.content.publish_status === "ready" ? "可发布" : "进入人工发布复核"}</FieldCard></div>
    </div>
  </SceneShell>;
};
